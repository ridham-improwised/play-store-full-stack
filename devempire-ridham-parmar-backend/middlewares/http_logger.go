...
func LogHandler(logger *zap.Logger, pMetrics *pMetrics.PrometheusMetrics) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		...
		switch {
		case ctx.Response().StatusCode() >= 200 && ctx.Response().StatusCode() < 300:
			pMetrics.RequestsMetrics.WithLabelValues("2xx").Inc()
		case ctx.Response().StatusCode() >= 300 && ctx.Response().StatusCode() < 400:
			pMetrics.RequestsMetrics.WithLabelValues("3xx").Inc()
		case ctx.Response().StatusCode() >= 400 && ctx.Response().StatusCode() < 500:
